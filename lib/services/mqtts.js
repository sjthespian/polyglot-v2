  ...
  */
  async start() {
    if (!config.mqttServer) {
      ...
      // Read custom key if both MQTT_HOST customSSL are set
      if (config.settings.mqttHost != 'localhost' && config.settings.customSSL) {
        options.ca = splitca(sslDir + 'custom/custom.ca', '\n', 'utf8')
        options.key = fs.readFileSync(sslDir + 'custom/custom.key', 'utf-8'),
        options.cert = fs.readFileSync(sslDir + 'custom/custom.crt', 'utf-8')
      }
      config.aedes = Aedes({
        //mq: mq,
        //persistence: persist,
