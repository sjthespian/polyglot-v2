...
const SettingsSchema = mongoose.Schema({
  ...
  'fqdn': {
    type: String,
    default: process.env.HOST_FQDN || (shell.exec("hostname -f", { silent: true }).stdout.trim() || 'localhost')
  },
  'bindIPAddress': {
    type: String,
...
SettingsSchema.statics = {
  ...
  async parseEnvUpdate() {
    try {
      ...
      if (settings.hasOwnProperty('HOST_FQDN')) { config.settings.fqdn = settings.HOST_FQDN } else {(shell.exec("hostname -f", { silent: true }).stdout.trim() || 'localhost') }
